...
```{python}
##1. ¿Que pasaría si eliminamos la verificación if cls._instancia is None en el método new?
##No existiría una única instancia dentro de la clase, y cada vez que se llame a la clase crearia una instancia nueva.

##2. ¿Que significa el "True" en el p1 is p2 is p3 en el contexto del método Singleton?
##Que en memoria todos son la misma instancia.

##3. ¿Es buena idea usar Singlenton para todo lo que se global? Menciona un ejemplo donde no seria recomendable.
##No no es recomendable, por ejemplo en un objeto de conexión de base de datos en una página web, porque al usar el
##singleton todos losusuarios compartirían la misma conexión, lo cual generaría problemas de rendimiento y saturación de la página.
```
